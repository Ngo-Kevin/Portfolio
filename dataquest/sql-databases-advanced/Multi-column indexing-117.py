...
query_plan_two= conn.execute('EXPLAIN QUERY PLAN SELECT * FROM facts WHERE population > 1000000 AND population_growth < .05;').fetchall() 
...
query_plan_four = conn.execute('explain query plan select population, population_growth from facts where population > 1000000 and population_growth < .05;').fetchall()
